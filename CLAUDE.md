...
- `public/js/anilist.js` — `Anilist.suggest(text, kind)` (name typeahead) and `Anilist.setImage(img, name, kind)` (illustration), both querying graphql.anilist.co **from the player's browser**, with a per-tab cache. This ran in the Worker until AniList answered it `403 "You have been manually blocked"`: Workers share a few egress IPs and AniList blacklists them, so no throttling or caching on our side helps. From the browser each player uses their own IP and AniList's CORS support. Rooms therefore no longer store image URLs in their state — the client resolves the picture from the name it is already displaying. `scripts/` has no live test for this (it needs the network); `npm test` stubs the endpoint.
