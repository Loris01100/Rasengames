...
- `public/js/anilist.js` — `Anilist.suggest(text, kind)` (name typeahead) and `Anilist.setImage(img, name, kind)` (illustration), both querying graphql.anilist.co **from the player's browser**, with a per-tab cache. This ran in the Worker until AniList answered it `403 "You have been manually blocked"`: Workers share a few egress IPs and AniList blacklists them, so no throttling or caching on our side helps. From the browser each player uses their own IP and AniList's CORS support. Rooms therefore no longer store image URLs in their state — the client resolves the picture from the name it is already displaying. When a player picks a suggestion, the exact entry travels with the word as an `anilistRef` (`"character:83801"`, validated server-side, stored as `wordRef`/`proposalRef`) and wins over the name search: searching "King" returns Lelouch, whose aliases include "Black King", not King of Nanatsu no Taizai. Hand-typed words have no ref and still fall back to the name. `scripts/` has no live test for this (it needs the network); `npm test` stubs the endpoint.
