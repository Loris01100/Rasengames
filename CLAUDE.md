...
node scripts/disconnect-test.mjs   # demande un `npm run dev` en face : hôte réattribué + phase débloquée quand un joueur part
```

`npm test` only covers the "même mot" rule and the frontends against a stub DOM. `scripts/disconnect-test.mjs` covers the server side but needs a live `npm run dev` (hence it is out of `npm test`): it drives real WebSocket clients through the disconnect paths that used to strand a salon. Beyond those, game logic and the protocol were validated during development with throwaway Node scripts (native `WebSocket`, simulating multiple players against `wrangler dev`) and a Playwright script for the drag-and-drop UI — neither is checked into the repo. When changing game logic, the fastest way to verify correctness is the same approach: run `npm run dev` and drive a couple of WebSocket clients through a full game.
