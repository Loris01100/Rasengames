...
export class WhoamiRoom {
  ...
  private async onStart(session: Session, room: RoomState, msg: Record<string, unknown>) {
    ...
    if (submitMode === "categorie") {
      ...
      if (!category) {
        sendError(session.ws, "Écris une catégorie, ou repasse en mode libre.");
        return;
      }
    }
  ...
  private async onSubmitWord(session: Session, room: RoomState, msg: Record<string, unknown>) {
    ...
    if (!text) {
      sendError(session.ws, "Mot invalide.");
      return;
    }
  ...
  private async onGuess(session: Session, room: RoomState, msg: Record<string, unknown>) {
    ...
    if (player.pendingGuess) {
      sendError(session.ws, "Ta proposition attend encore la validation.");
      return;
    }
  ...
  private async onValidateGuess(session: Session, room: RoomState, msg: Record<string, unknown>) {
    ...
    if (!this.canValidate(room, session.playerId, targetId)) {
      sendError(session.ws, "Seul l'hôte valide les propositions.");
      return;
    }
  ...
  // Un point par joueur qui a trouvé son personnage, cumulé sur le salon.
  private awardFound(room: RoomState) {
    for (const p of Object.values(room.players)) {
      if (p.found) room.scores[p.id] = (room.scores[p.id] ?? 0) + 1;
    }
  }
